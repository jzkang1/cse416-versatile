...
function GlobalStoreContextProvider(props) {
	...
	store.startEditMap = async function(mapId) {
		console.log("store.startEditMap")
		let payload = {
			_id: mapId
		}
		let response = await api.startEditMap(payload);
		if (response.data.success) {
			console.log("start edit map success")
	...
	store.likeMap = async function() {
		...
		let response = await api.likeMap({
			_id,
			username
		});
		if (response.data.success) {
	...
	store.unlikeMap = async function() {
		...
		let response = await api.unlikeMap({
			_id,
			username
		});
		if (response.data.success) {
	...
	store.dislikeMap = async function() {
		...
		let response = await api.dislikeMap({
			_id,
			username
		});
		if (response.data.success) {
	...
	store.undislikeMap = async function() {
		...
		let response = await api.undislikeMap({
			_id,
			username
		});
		if (response.data.success) {
	...
	store.deleteMap = async function(mapId) {
		...
		console.log(mapId);
		let response = await api.deleteMap({
			_id: mapId
		});
		if (response.data.success) {
			store.loadPersonalMaps();
	...
	store.duplicateMap = async function(mapId) {
		...
		console.log(mapId);
		let response = await api.duplicateMap({
			_id: mapId
		});
		if (response.data.success) {
			store.loadPersonalMaps();
